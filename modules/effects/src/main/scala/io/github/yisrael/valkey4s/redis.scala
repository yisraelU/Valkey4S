...
object Redis {
  ...
  private[valkey4s] def acquireAndRelease[F[_]: FutureLift: Log: MonadThrow, K, V](
      client: RedisClient,
  ...
  private[valkey4s] def acquireAndReleaseCluster[F[_]: FutureLift: Log: MonadThrow, K, V](
  ...
  private[valkey4s] def acquireAndReleaseClusterByNode[F[_]: FutureLift: Log: MonadThrow, K, V](
      client: RedisClusterClient,
      codec: RedisCodec[K, V],
      readFrom: Option[JReadFrom],
      nodeId: NodeId,
...
private[valkey4s] class BaseRedis[F[_]: FutureLift: MonadThrow: Log, K, V](
...
private[valkey4s] class Redis[F[_]: FutureLift: MonadThrow: Log, K, V](
    connection: RedisStatefulConnection[F, K, V],
    tx: TxRunner[F]
) extends BaseRedis[F, K, V](connection, tx, cluster = false)

private[valkey4s] class RedisCluster[F[_]: FutureLift: MonadThrow: Log, K, V](
