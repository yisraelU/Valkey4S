...
class SortedSetCommandsSuite extends ValkeyTestSuite {
  ...
  test("ZREM should remove members from sorted set") {
    valkeyClient.use { valkey =>
      for {
        _ <- valkey.zadd("zset3", Map("a" -> 1.0, "b" -> 2.0, "c" -> 3.0, "d" -> 4.0))
  ...
  test("ZRANGE should return members in score order") {
    valkeyClient.use { valkey =>
      for {
        _ <- valkey.zadd("zset4", Map("alice" -> 100.0, "bob" -> 50.0, "charlie" -> 150.0))
  ...
  test("ZRANGE should return subset by index") {
    valkeyClient.use { valkey =>
      for {
        _ <- valkey.zadd("zset5", Map("a" -> 1.0, "b" -> 2.0, "c" -> 3.0, "d" -> 4.0, "e" -> 5.0))
  ...
  test("ZRANGEWITHSCORES should return members with scores") {
    valkeyClient.use { valkey =>
      for {
        ...
        assertEquals(membersWithScores, List(("a", 1.5), ("b", 2.5), ("c", 3.5)))
      }
    }
  }
  ...
  test("ZCARD should return number of members") {
    valkeyClient.use { valkey =>
      for {
        _ <- valkey.zadd("zset10", Map("a" -> 1.0, "b" -> 2.0, "c" -> 3.0, "d" -> 4.0))
  ...
  test("ZRANK should return rank of member (0-based, ascending)") {
    valkeyClient.use { valkey =>
      for {
        _ <- valkey.zadd("zset11", Map("a" -> 1.0, "b" -> 2.0, "c" -> 3.0, "d" -> 4.0))
  ...
  test("ZREVRANK should return rank in descending order") {
    valkeyClient.use { valkey =>
      for {
        _ <- valkey.zadd("zset13", Map("a" -> 1.0, "b" -> 2.0, "c" -> 3.0, "d" -> 4.0))
  ...
  test("ZCOUNT should count members in score range") {
    valkeyClient.use { valkey =>
      for {
        _ <- valkey.zadd("zset16", Map("a" -> 1.0, "b" -> 2.0, "c" -> 3.0, "d" -> 4.0, "e" -> 5.0))
  ...
  test("ZPOPMINCOUNT should remove and return multiple members with lowest scores") {
    valkeyClient.use { valkey =>
      for {
        _ <- valkey.zadd("zset18", Map("a" -> 1.0, "b" -> 2.0, "c" -> 3.0, "d" -> 4.0))
  ...
  test("ZPOPMAXCOUNT should remove and return multiple members with highest scores") {
    valkeyClient.use { valkey =>
      for {
        _ <- valkey.zadd("zset20", Map("a" -> 1.0, "b" -> 2.0, "c" -> 3.0, "d" -> 4.0))
  ...
  test("ZRANDMEMBERCOUNT should return multiple random members") {
    valkeyClient.use { valkey =>
      for {
        _ <- valkey.zadd("zset22", Map("a" -> 1.0, "b" -> 2.0, "c" -> 3.0, "d" -> 4.0))
  ...
  test("complex workflow: leaderboard system") {
    valkeyClient.use { valkey =>
      for {
        ...
        // Get top 3 players
        top3 <- valkey.zrangeWithScores("leaderboard", -3, -1) // Last 3 (highest scores)
  ...
  test("complex workflow: priority queue") {
    valkeyClient.use { valkey =>
      for {
        ...
        assertEquals(remaining.map(_._1), List("feature-request", "documentation"))
      }
    }
  }
}
